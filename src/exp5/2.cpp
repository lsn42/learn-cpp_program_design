...
Teacher::Teacher(int num, char name[], char sex):
  num(num), sex(sex) //请给出Teacher类的构造函数
{
...
BirthDate::BirthDate(int year, int month, int day):
  year(year), month(month), day(day){}; //请给出BirthDate类的构造函数
...
Professor::Professor(
  int num, char name[], char sex, int year, int month, int day, float area):
  Teacher(num, name, sex), birthday(year, month, day), area(area){};
